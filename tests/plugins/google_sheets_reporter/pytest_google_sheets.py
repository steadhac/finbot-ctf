...
def extract_iso_code(docstring: Optional[str]) -> Optional[str]:
    """Extract test code from docstring (ISO-*, SSM-*, SAI-*, RDS-*, etc.)."""
    if not docstring:
        return None
    # Match patterns like: ISO-DAT-001, SSM-HMC-001, BAF-SSN-001, SAI-INV-001,
    # SAI-EDGE-001, SAI-GSI-001, RDS-RED-001, RDS-HEA-005, etc.
...
def detect_test_category(item) -> str:
    ...
    # NOTE: 'redis_message_streams' must come before 'agents' so Redis tests
    #       route to their own tab instead of falling into Base Agent Framework.
    #       'specialized' must also come before 'agents' for the same reason.
    path_worksheet_map = [
